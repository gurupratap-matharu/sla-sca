...
exports.sla_extract_patch = function(img) { //import image to extract SLA
...
var areacalc = function(image4) { //function to calculate the area of an image 
...
var forvec = classified.mask(snowmask1).select('classification').multiply(0).unmask(-10).clip(geometry);
var forvec1 = classified.mask(icemask1).select('classification').add(10).unmask(-10).clip(geometry);
...
var voidpart =      ee.Number(1).subtract((ee.Number(areaice).add(ee.Number(areasnow))).divide(areacalc(elevation)));
...
var notouch = function(){
  var lowestsnowelev = function(){
    var numb = elevation.clip(maxsnow).reduceRegion({
                     reducer: ee.Reducer.min(),
                      geometry: geometry,
                    scale : 30,
                     bestEffort: true
...
{
  var conditionalXXX = ee.Algorithms.If(sl1,        //check, if SL1 is null or not (e.g. if theres no ice-pixel)
          sl1, notouch());
...
var areasnowpatch =   ee.Algorithms.If(maxsnow1.size(),ee.Number(maxsnow.get('count')),0);
...
var touchingzone = biggerice.subtract(img2cl.mask(img2cl.select('first').gte(9)));
...
feature = feature.set('Considered Area for MP-SLA Extraction', consideredareapatches);
feature = feature.set('stdDev SLA, 0 means no touch between Main patches [m]', stdDevSLA);
