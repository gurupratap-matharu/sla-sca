...
exports.decisiontree = function (img) {
  ...
  var Yaddhillshadow = function () {
    //function to add a layer with the hillshadow
  ...
  var Naddhillshadow = function () {
    //function to add a dummy-layer with <-1> value for every pixel
  ...
  var addhillshadow = ee.Algorithms.If(
    hsboolean,
    Yaddhillshadow(),
    Naddhillshadow()
  );
  ...
  var hillshadowmask = img4.select("shadow").eq(0).clip(geometry);
  ...
  // Snow is reasonably bright in all visible bands.
  score = score.min(
    rescale(icesnowdif, "img.red + img.green + img.blue", [0.5, 2])
  );
  ...
  var points = 100;
  var randomPointsice = ee.FeatureCollection.randomPoints(icevec, points).map(
    function (feat) {
      return feat.set("cover", 0);
    }
  );
  var randomPointssnow = ee.FeatureCollection.randomPoints(snowvec, points).map(
    function (feat) {
      return feat.set("cover", 1);
    }
  );
  var randomPointsdebris = ee.FeatureCollection.randomPoints(
    debrisvec,
    points
  ).map(function (feat) {
    return feat.set("cover", 3);
  });
  var randomPointswater = ee.FeatureCollection.randomPoints(
    watervec,
    points
  ).map(function (feat) {
    return feat.set("cover", 2);
  });
  var randomPointsclouds = ee.FeatureCollection.randomPoints(
    cloudsvec,
    points
  ).map(function (feat) {
    return feat.set("cover", 4);
  });
  var randomPointshadsnow = ee.FeatureCollection.randomPoints(
    shadsnowvec,
    points
  ).map(function (feat) {
    return feat.set("cover", 6);
  });
  var randomPointshadow = ee.FeatureCollection.randomPoints(
    shadowvec,
    points
  ).map(function (feat) {
  ...
  var fc1 = randomPointsice
    .merge(randomPointssnow)
    .merge(randomPointsdebris)
    .merge(randomPointswater)
    .merge(randomPointsclouds)
    .merge(randomPointshadsnow)
    .merge(randomPointshadow);
  ...
  var classified = classifiedRF
    .set("system_time_start", systemtime)
    .set("ING_ID", ing_id)
    .set("otsu", nirthreshold)
    .set("deminfo", deminfo)
    .clip(geometry)
    .set("metadata", fcmetadata)
