...
def project_shadows(image, cloud_mask, cloud_heights, contract_pixels, dilate_pixels, infrared_threshold):
    ...
    dark_pixels = image.select(['nir', 'swir1', 'swir2']).reduce(ee.Reducer.sum()).lt(infrared_threshold)
    ...
    azimuth_radians = ee.Number(mean_azimuth).multiply(math.pi).divide(180.0).add(ee.Number(0.5).multiply(math.pi))
    zenith_radians  = ee.Number(0.5).multiply(math.pi).subtract(ee.Number(mean_zenith).multiply(math.pi).divide(180.0))
