...
exports.ABextraction = function (imgprep) {
  ...
  var getlevelinfo = function (level) {
    ...
    var totalsnow = areasnow.add(
      areasnow.divide(totalsnowice).multiply(areavoid)
    );

    var totarea = totalice.add(totalsnow);
    var ratiototalsnow = areasnow
      .add(areasnow.divide(totalsnowice).multiply(areavoid))
      .divide(arealevel);
  ...
  var addsla = function (nmbr) {
    ...
    var first = ee
      .Number(
        FC.filterMetadata("level lower boundary", "equals", startnmr)
          .first()
    ...
    var second = ee
      .Number(
        FC.filterMetadata("level lower boundary", "equals", secondfilter)
          .first()
          .get("ratio totalsnow (SCR)")
      )
      .gt(0.5);
    var third = ee
      .Number(
        FC.filterMetadata("level lower boundary", "equals", thirdfilter)
          .first()
          .get("ratio totalsnow (SCR)")
      )
      .gt(0.5);
  ...
  var slacondition = ee.Number(
    ee.Algorithms.If(
      condition,
      ee.Number(slaf.get("level lower boundary")),
      ee.Number(-9999)
    )
  );
  ...
  var alternativesla = function () {
    var altsla01 = FC.filterMetadata(
      "ratio totalsnow (SCR)",
      "greater_than",
      0.4999999
    ).size();
    var altsla02 = FC.filterMetadata(
      "ratio totalsnow (SCR)",
      "greater_than",
      0.4999999
    )
