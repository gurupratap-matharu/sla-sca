...
exports.imageCollection = function(glimsid, startyear, endyear, cloudiness, coverage, filterDOYstart,filterDOYend,hsboolean,dem) {

//////////////////    RENAME BANDS    /////////////////////////////////////////
var s2_bands = ee.List([ 'B1','B2','B3','B4','B5','B6','B7','B8','B8A', 'B9','B10', 'B11','B12', 'QA60']);
var s2_band_names = ee.List(['cb', 'blue', 'green', 'red', 're1','re2','re3','nir', 're4', 'vapor', 'cirrus','swir1', 'swir2', 'BQA']);

var l8_bands = ee.List([ 'B1','B2','B3','B4','B5','B6','B7','B10', 'B11','BQA']);
var l8_band_names = ee.List(['cb', 'blue', 'green', 'red', 'nir','swir1','swir2', 'tir1','tir2','BQA']);

var l7_bands = ee.List([ 'B1','B2','B3','B4','B5','B6_VCID_1','B7','BQA']);
var l7_band_names = ee.List(['blue', 'green', 'red', 'nir','swir1','tir1','swir2','BQA']);
 
var l5_bands = ee.List([ 'B1','B2','B3','B4','B5','B6','B7','BQA']);
var l5_band_names = ee.List(['blue', 'green', 'red', 'nir','swir1','tir1','swir2','BQA']);
///////////////////   PREPARE AND FILTER RGI INVENTORY V6   /////////////////////

var geometry = RGI.filterMetadata('GLIMSId','equals',glimsid); //filter the rgi inventory with the input of the glimsID --> get the geometry
...
var areacalcrast = function(img) { 
                ...
                var fc1 = img1.reduceToVectors({ //produces a FeatureCollection
...
var areacalc = function(image4) { //function to calculate the area of an image (area of a band without masked pixels)
...
var areatotal1 = ee.Image(1).clip(geometry).select('constant').rename('green');
...
var addAlbedo = function(img) {
  var albedo = (((        img.select('blue').multiply(0.356))
...
var addquainfo = function(data) { //data is one image from the imagecollection (unfiltered)
                    ...
                    var image1 = image  .set('GLIMSID', glimsid) //set glimsid to image
...
var saddquainfo = function(data) {
                    ...
                    var areaRGIoutline = areacalcrast(clip);
                    var areaIMGglacier = areaIMGglacier1;
                    var arearatio = areaRGIoutline.divide(areaIMGglacier).multiply(100);
                    ...
                    var image1 = image.set('GLIMSID', glimsid)
                        .set('areaRGIoutline',areaRGIoutline)
                        .set('areaIMGglacier',areaIMGglacier)
                        .set('arearatio',arearatio)
...
var landsat5data = l5.filterBounds(geometry) //filter on the base of the choosen geometry
                .filterDate(startdate, enddate)
                .filter(ee.Filter.calendarRange(filterDOYstart,filterDOYend,'day_of_year'))
                .map(function(img) { //create image collection
...
var landsat7data = l7.filterBounds(geometry)
...
var landsat8data = l8.filterBounds(geometry)
                .filterDate(startdate, enddate).filter(ee.Filter.calendarRange(filterDOYstart,filterDOYend,'day_of_year'))
                .map(function(img) {
...
var sentineldata = s2.filterBounds(geometry)
                .filterDate(startdate, enddate).filter(ee.Filter.calendarRange(filterDOYstart,filterDOYend,'day_of_year'))
                .map(function(img) {
...
var imgcollfiltered = imgcoll_added.filter(ee.Filter.eq("status_dublicate","no_dublicate"));
return imgcollfiltered;
};
var noimage = function(){
        //  Create Empty ImageCollection
            var emptyimgcol = ee.ImageCollection(ee.Image(5)).filter(ee.Filter.eq('system:index', 0));
  return emptyimgcol;
};
...
var landsat5datafi = ee.ImageCollection(startdublicatfiltering(landsat5datapre));
var landsat7datafi = ee.ImageCollection(startdublicatfiltering(landsat7datapre));
var landsat8datafi = ee.ImageCollection(startdublicatfiltering(landsat8datapre));
var sentineldatafi = ee.ImageCollection(startdublicatfiltering(sentineldatapre));

// Merge all individual collections per Sensor to one collection
var imagecol_all = landsat5datafi.merge(landsat7datafi).merge(landsat8datafi).merge(sentineldatafi);
