...
var lowsla1 = elevation.reduceRegion({
      reducer: ee.Reducer.min(),
      geometry: geometry,
      scale : 30
      }).get('AVE_DSM');

var highsla = elevation.reduceRegion({
...
var elevclassice = elevation.mask(classified.select('classification').clip(geometry).eq(0));
var elevclassnow = elevation.mask(classified.select('classification').clip(geometry).eq(1));
...
var YYY = function(){
var lowsla = elevation.reduceRegion({
      reducer: ee.Reducer.min(),
      geometry: geometry,
      scale : 30
      }).get('AVE_DSM');
...
var XXX = function(elevclass)  {
  var hist_snow = elevation.updateMask(classified.select('classification').eq(1)).reduceRegion({ //calculate histogram for snow
  ...
  var features_snow = hist_snow_array.map(function(item) {
    item = ee.List(item);
    return ee.Feature(null, {elevation: item.get(0), countsnow: item.get(1) });
  });

  
  var hist_ice = elevation.updateMask(classified.select('classification').eq(0)).reduceRegion({ //calculate histogram for ice. classification 0 is ice
    reducer: ee.Reducer.autoHistogram(40, 50), 
    geometry: geometry,
    scale: 30,
    maxPixels: 1e13
  ...
  var features_ice = hist_ice_array.map(function(item) {
    item = ee.List(item);
    return ee.Feature(null, {elevation: item.get(0), countice: item.get(1) });
  });


var fcFilter = ee.Filter.equals({ // create filter to use it for the inner join of the two FeatureCollections (snow and ice)
  leftField: 'elevation',
...
var calcfc = fcJoin.map(function(i) { //map function to calculate the ration between sum of pixels and the difference of pixels over featurecollection
           ...
           var snowfeatdif = snowfeat.set('dif',dif).set('countice', ice).set('ratio', ratio).set('sum',sum); //write all properties in one feature
...
var calcarea1 = function(image4) { //function to calculate the area of an image (area of a band without masked pixels)
...
var condslafin = calcarea1((classified.select('classification').clip(geometry).eq(0)));
