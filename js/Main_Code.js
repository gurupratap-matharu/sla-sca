...
Map.onClick(function (coords) {
  ...
  print(
    "GLIMS ID: ",
    ee.String(RGI.filterBounds(click_point).first().get("GLIMSId"))
  );
});
    ...
    var button = ui.Button({
      ...
      onClick: function () {
        ...
        Map.addLayer(
          prepfinal
            .first()
            .reproject(prepfinal.first().select("blue").projection()),
        ...
        var buttonDT = ui.Button({
          ...
          onClick: function () {
            ...
            Map.add(legend);
            Map.addLayer(
              mapCollection.first().select("classification"),
              { min: 0, max: 8, palette: palette },
              "Classification of the first Image in the Collection"
            );
