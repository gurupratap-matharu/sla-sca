...
Map.onClick(function (coords) {
  ...
  print(
    "ING ID: ",
    ee.String(ING.filterBounds(click_point).first().get("ID_local"))
  );
});
...
var hsboolean = 0;
var resultPRE = composites.imageCollection(
  glimsid,
  startyear,
  endyear,
  cloudiness,
...
var result = resultPRE.filterMetadata(
  "system:time_start",
  "not_equals",
  1443176819706
); //filter out this image due to wrong scaled bands
...
var prepfinal = ee.ImageCollection(
  imgcolN.merge(imgcolY).sort("system:time_start")
);
...
//print(prepfinal);
Map.addLayer(
  prepfinal.first().reproject(prepfinal.first().select("blue").projection()),
  { bands: ["red", "green", "blue"], min: 0, max: 1 },
  "First available Image in this Collection"
);
...
Map.add(legend);
Map.addLayer(
  mapCollection.first("false").select("classification"),
  { min: 0, max: 8, palette: palette },
  "Classification of the last Image in the Collection"
);
...
Export.image.toDrive({
  image: exampletif,
  scale: 20,
  description: "imageExport_Alerce19",
});
...
Export.table.toDrive(
  ABcollection,
  "AltitudeLevelsApproach_ALERCE19",
  "slamontitoringdata_test_Alerce"
);
