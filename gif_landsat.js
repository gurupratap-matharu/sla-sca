...
var L4 = ee.ImageCollection("LANDSAT/LT04/C02/T1_L2"),
    ...
    ee.Geometry.Polygon(
        [[[-113.31908262896587, 41.80375175369021],
          ...
          [-111.80949248004009, 41.80375175369021]]], null, false),
...
var VP = {opacity:1,bands:["R","G","B"],min:[8750,9463,8873],max:[20000,16000,13937],gamma:2};
var VP = {opacity:1,bands:["NIR","R","G"],min:[9735,7950,7882],max:[19797,15544,12904],gamma:2};  //st George
var VP = {opacity:1,bands:["R","G","B"],min:7355,max:21826,gamma:2};
var VP = {opacity:1,bands:["SWIR2","NIR","R"],min:[7088,7175,8091],max:[26844,28366,30398],gamma:2};
...
var l9 = L9
  .filterBounds(fc).map(masksr)
  .map(function (image){return image.select(l8bands).rename(bndNames)})
var l8 = L8
  .filterBounds(fc).map(masksr)
  .map(function (image){return image.select(l8bands).rename(bndNames)})
var l7 = L7
  .filterBounds(fc).map(masksr)
  .map(function (image){return image.select(l57bands).rename(bndNames)})
var l5 = L5
  .filterBounds(fc).map(masksr)
  .map(function (image){return image.select(l57bands).rename(bndNames)})
var l4 = L4
...
var TMmerged = l4.merge(l5).merge(l7).merge(l8).merge(l9).sort("system:time_start");

//  Generate a hillshade to place under the NDVI
var HillShade = ee.Terrain.hillshade(DEM.clip(fc).multiply(2),135, 45).divide(255);

// set annotation parameters and label location
print ('Scale',Map.getScale())
var annotations = [
  {position: 'left', offset: '1%', margin: '1%', property: 'label', scale: dist.divide(100) }//Map.getScale() * 2
];
...
var Images = ee.ImageCollection.fromImages(
  allYears.map(function (Y) {
    ...
    var image1 = TMmerged.filter(ee.Filter.calendarRange(year.subtract(1),year.subtract(1), 'year'))
                 .filter(ee.Filter.calendarRange(startMonth,endMonth, 'month'))
                 .median().clip(fc);
    var image2 = TMmerged.filter(ee.Filter.calendarRange(Y,Y, 'year'))
                 .filter(ee.Filter.calendarRange(startMonth,endMonth, 'month'))
                 .median().set('year',Y).clip(fc);
...
var sequence = Images.map(function (image){
    ...
    var theImage = image.visualize(VP).multiply(HillShade)
                        .set({label: ee.Number(year).int()});
    var annotated = text.annotateImage(theImage, {}, LabelLocation, annotations).set('year',year);
    return annotated;
});
